...
class ExampleTestRunner:
    ...
    async def setup_test_database(self):
        ...
        try:
            ...
            for symbol in self.test_symbols:
                # Save trades
                async with TradeRepository(self.test_exchange, symbol, test=True) as repo:
                ...
                # Save candles  
                async with CandleRepository(self.test_exchange, symbol, test=True) as repo:
                    success = await repo.save_candles(test_candles)
                    if success:
                        print(f"✅ Created {len(test_candles)} test candles for {symbol}")
                    else:
    ...
    async def run(self, specific_example=None, setup_only=False, cleanup_only=False):
        ...
        title = f"🧪 fullon_ohlcv_api Examples Test Suite"
        if specific_example:
        ...
        if not setup_only:
            print("\n" + "=" * 50)
            if success:
                print("🎉 All examples passed!" if not specific_example else f"🎉 {specific_example} passed!")
                return 0
            else:
                print("❌ Some examples failed" if not specific_example else f"❌ {specific_example} failed")
                return 1
